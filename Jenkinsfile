pipeline {
  environment {
    ARGO_SERVER = '34.132.136.125:32100'
    DEV_URL = 'http://34.132.109.169:30080/'
  }
  agent {
    kubernetes {
      ...
      parallel {
        stage('Create Jarfile') {
	      ...
	      stage('OCI Image BnP') {
	        steps {
	          container('kaniko') {
	            sh '/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --insecure --skip-tls-verify --cache=true --destination=docker.io/trate/dso-demo'
	          }
	        }
	      }
      }
       ...
       stage('Image Linting') {
         steps {
           container('docker-tools') {
             sh 'dockle docker.io/trate/dso-demo'
           }
         }
       }
       stage('Image Scan') {
